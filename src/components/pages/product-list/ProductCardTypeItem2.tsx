...
export default function ProductCardTypeItem2({ productId }: { productId: number }) {
    ...
    function handleClick() {
        if (confirm('상품 상세에서 옵션을 선택 후 장바구니 담기를 진행해주세요!')){
            router.push(`/product-detail?productId=${productId}`);
        }
        return
    }
        ...
        <div className='pt-[10px] pb-[20px]'>
            ...
            <div className='flex mt-[3px] py-[2px]'>
                ...
                <div className='flex'>
                    ...
                    {/* <Like w={20} h={20} /> */}
                    {/* <Link href={`/product-detail?productId=${productId}`} className='mr-[3px]'><CartIcon w={26} h={26} /></Link> */}
                    <div className='mr-[3px]' onClick={handleClick}><CartIcon w={26} h={26} /></div>
                </div>
            </div>
